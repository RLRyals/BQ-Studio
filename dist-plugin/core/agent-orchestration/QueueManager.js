...
class QueueManager {
     ...
     */
    async processQueue() {
        while (this.queue.currentRunning < this.queue.maxConcurrent &&
    ...
    async failJob(jobId, error) {
        ...
        if (job.retryCount < job.maxRetries && error.recoverable) {
            ...
            const delay = Math.min(this.config.retryDelay * Math.pow(this.config.backoffMultiplier, job.retryCount - 1), this.config.maxRetryDelay);
            setTimeout(() => {
                this.processQueue();
            }, delay);
        }
     ...
     * Update job progress
     */
    updateJobProgress(jobId, progress, currentPhase) {
     ...
     * Add log to job
     */
    addLog(jobId, level, message, source = 'system', metadata) {
        const job = this.findJob(jobId);
        if (job) {
            job.logs.push({
                timestamp: new Date(),
     ...
     */
    updateTokenUsage(jobId, inputTokens, outputTokens) {
        const job = this.findJob(jobId);
        if (job) {
     ...
     */
    registerExecutionCallbacks(jobId, resolve, reject) {
     ...
     */
    findJob(jobId) {
        return (this.queue.pending.find((j) => j.id === jobId) ||
    ...
    cleanup(retentionDays = 30) {
        ...
        this.queue.completed = this.queue.completed.filter((job) => job.completedAt && job.completedAt > cutoffDate);
        this.queue.failed = this.queue.failed.filter((job) => job.lastActivityAt && job.lastActivityAt > cutoffDate);
    }
}
